...
export async function POST(req: NextRequest) {
  try {
    ...
    const serviceName = String(service).replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase())
    const subject = `New ${serviceName} Quote Request – ${firstName} ${lastName} – ${city || 'Location TBD'}`
    ...
    const teamRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        reply_to: email,
        subject: isDev ? `[DEV TEST] ${subject}` : subject,
        html: buildTeamEmail(body, isDev),
      }),
    })

    if (!teamRes.ok) {
      console.error('Resend error:', await teamRes.text())
      return NextResponse.json({ error: 'Failed to send notification' }, { status: 500 })
    }

    // Auto-reply to client (production only)
    if (!isDev) {
...
function buildTeamEmail(data: Record<string, unknown>, isDev: boolean): string {
  const devBanner = isDev ? '<div style="background:#ef4444;color:white;padding:8px 16px;font-family:monospace;font-size:12px;margin-bottom:16px">⚠️ DEV TEST — sent to protonmail only — not to team</div>' : ''
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="background:#050a0f;color:#f0ece4;font-family:sans-serif;margin:0;padding:32px">
  ${devBanner}
  <div style="max-width:600px;margin:0 auto">
    <div style="border-left:3px solid #1ecfc9;padding-left:20px;margin-bottom:28px">
      <p style="font-family:monospace;font-size:10px;letter-spacing:3px;color:#1ecfc9;margin:0 0 6px">NEW QUOTE REQUEST · BRE BUILDERS</p>
    ...
    <table style="width:100%;border-collapse:collapse;background:#0f1e2d;border-radius:12px;overflow:hidden">${rows}</table>
    <div style="margin-top:24px;padding:16px;background:#0f1e2d;border-radius:8px;border:1px solid rgba(255,255,255,0.07)">
      <p style="margin:0;font-size:12px;color:#8a95a3">Reply to: ${data.email} · ${new Date().toLocaleString('en-US', { timeZone: 'America/Los_Angeles' })} PT</p>
    </div>
  </div></body></html>`
}
...
function buildClientEmail(data: Record<string, unknown>): string {
  ...
  <div style="max-width:560px;margin:0 auto">
    ...
    <p style="color:#8a95a3;margin:0 0 20px">Blue Reef Builders</p>
