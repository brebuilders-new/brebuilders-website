...
function Step4Service({ form, set }: { form: FormState; set: (k: keyof FormState, v: unknown) => void }) {
  const s = form.service

  const ServiceLabel = () => {
    const svc = SERVICES.find(x => x.id === s)
    return <Label>Step 4 of 6 — {svc?.label || 'Project details'}</Label>
  }
...
function Step5Budget({ form, set }: { form: FormState; set: (k: keyof FormState, v: unknown) => void }) {
  const ranges = BUDGET_RANGES[form.service as ServiceId] || BUDGET_RANGES['other']
...
function Step6Confirm({
  ...
  const summaryRows: { label: string; value: string }[] = [
    { label: 'Service', value: svc?.label || form.service },
...
export default function SmartQuoteForm({ preselect, className = '' }: SmartQuoteFormProps) {
  ...
  const steps = [
    ...
    <Step4Service key={3} form={form} set={set} />,
    <Step5Budget key={4} form={form} set={set} />,
    <Step6Confirm key={5} form={form} set={set} submitting={submitting} error={error} />,
  ]
